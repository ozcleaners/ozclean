                    ...
                    <div class="float-end">
                        ...
                        @else
                            <a class="d-inline-block alert-success py-0 px-2"
                               href="{{route('common_term_custom_field_breakdown_add', [request()->id]) }}?zone={{ $zone }}&custom_field_id={{ $value->id }}">
                                List of {{ $value->content_type }}
                            </a>
                        @endif
                        @if(auth()->user()->hasRoutePermission('common_term_custom_field_store'))
                            {!! $ButtonSet::delete('common_term_custom_field_destroy', $value->id) !!}
                            <a class="d-inline-block py-0"
                               href="{{ route('common_term_custom_field_edit', [request()->id]) }}?zone={{ $zone }}&section_id={{ $value->id }}">
