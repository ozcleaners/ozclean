    ...
    <tr>
        <td colspan="2">
            ...
            <p>
                ...
                @endphp
                @if($address)
                {!! $address['address'] !!}, {!! $address['state'] !!}, {!! $address['post_code'] !!}<br>
                @endif

                Email: {{$booking_general_info->email_address}}<br>
                Contact: {{$booking_general_info->contact_no}}
            </p>
        </td>
...
<table>
    @include('email.view-order-items', ['hash_code' => $booking_general_info->hash_code, 'add_service_in_order' => true])
</table>
