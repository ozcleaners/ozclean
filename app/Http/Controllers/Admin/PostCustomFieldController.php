...
class PostCustomFieldController extends Controller
{
...
//        dd($request);
        ...
        if ($validator->fails()) {
            ...
            $attributes = [
                'content_for' => 'Post',
                'content_term_id' => $request->get('content_term_id'), //66
                'content_term_parent_id' =>  $this->Model('Post')::getCat($request->get('content_term_id')), //Term::where('id', $request->get('content_term_id'))->first()->parent ?? NULL, //1
...
//            dd($attributes);
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view('admin.common.posts.post_custom_field')
            ->with(['termcustomfield' =>  $termcustomfield, 'post' => $post]);
    ...
    {
        return view('admin.common.posts.custom_field_breakdown');
    }
    ...
    public function custom_field_breakdown_store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                //dd($done);
                $r = url("/") . "/admin/common/post/custom_field_breakdown_edit/" . $request->content_term_id . "?zone=" . request()->get('zone') . "&custom_field_id=" . $request->term_custom_field_id;
    ...
    public function breakdown_update(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                //dd($done);
                $r = url("/") . "/admin/common/post/custom_field_breakdown_edit/" . $request->content_term_id . "?zone=" . request()->get('zone') . "&custom_field_id=" . $request->term_custom_field_id . "&custom_field_breakdown_id=" . $d->id;
                return redirect($r)->with(['status' => 1, 'message' => 'Successfully save changed']);
            } catch (\Illuminate\Database\QueryException $ex) {
                return redirect()->back()->withErrors($ex->getMessage());
            }
