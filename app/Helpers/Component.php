...
class Component{
    ...
    public static function confirmModal($modal_id, $form_id, $title="", $subtitle, $modal_body="", $submit_button = true){
        ...
        $html = '<div class="modal fade" style="background: #ffffffe6" id="'.$modal_id.'" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data--bs-keyboard="false" data-bs-backdrop="static">
    ...
    public static function jsModal($buttonId, $options = []){
        ...
        ?>
            ...
            <?php echo csrf_field() ?>
            <?php endif ?>
                <div class="modal fade" style="background: <?php echo $arr['modalBg'];?>" id="<?php echo $arr['modalId'];?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false" >
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content modal-alert">
                            <div class="modal-body text-center">
    ...
    public static function bootstrapModal($buttonId, $options = []){
        ...
        ?>
            ...
            <?php echo csrf_field() ?>
            <?php endif ?>
                <div class="modal <?php echo $arr['position'] ?> <?php echo $arr['modalId'];?> fade <?php echo $arr['backshadow'] ? 'modalBackdrop' : null;?> " id="<?php echo $arr['modalId'];?>" tabindex="-1"
    ...
    public static function modalScriptLoad($buttonId, $options = []){
        ...
        $use = $arr['use'] == 'id' ? '#' : '.';
         if($arr["btnWrapperId"]): ?>
            <script>
                jQuery('#<?php echo $arr["btnWrapperId"] ;?>').on('click',   '#<?php echo $buttonId ?>', function(e){
                // $(btns).click(modalBtn, function(e){
                    e.preventDefault();
                    jQuery('#<?php echo $arr['modalId'];?>').modal("show");
                })
            </script>
            <?php else :?>
            <script>
                jQuery(document).on('click', '<?php echo $use;?><?php echo $buttonId ?>', function(e){
                // $('<?php //echo $use;?><?php //echo $buttonId ?>').on('click', function(e){
                // $(btns).click(modalBtn, function(e){
                    e.preventDefault();
                    //alert('pk')
                    jQuery('<?php echo $use;?><?php echo $arr['modalId'];?>').modal("show");
                })
            </script>
            <?php endif;
    ...
    public static function sidebarModal($buttonId, $options=[]){
            ...
            $arr = array_merge($default, $options);?>
            <div class="modal left fade" id="<?php echo $arr['modalId'] ?>" tabindex="" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
